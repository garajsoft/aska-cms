...
import { buildTypographyHeadHtml, TYPOGRAPHY_DEFAULTS } from "@/lib/settings/typography";
...
export async function generateMetadata(): Promise<Metadata> {
  // Fail-soft: branding columns are missing while the schema lags the code
  // (e.g. mid-deploy before the push) — fall back to /favicon.ico instead
  // of 500ing every page.
  const { favicon } = await getBrandingAssets().catch(() => ({ favicon: undefined }));
  return {
...
export default async function RootLayout({ children }: LayoutProps<"/">) {
  ...
  const [tokensCss, snippets, typography, theme] = await Promise.all([
    ...
    // Same fail-soft as above — unconfigured system-font stack, not a 500.
    getTypographySettings().catch(() => ({
      ...TYPOGRAPHY_DEFAULTS,
      googleFontUrl: "",
      customFontLibrary: [],
      activeCustomFontId: "",
